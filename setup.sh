...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT} (C)$GRAY https://github.com/KARLTECHH$NC   │
    └───────────────────────────────────────────────┘
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/KARLTECHH/Register/main/reg | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/KARLTECHH/Register/main/reg | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/KARLTECHH/Register/main/reg | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/KARLTECHH/vip/main/"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/KARLTECHH/Register/main/reg | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/KARLTECHH/Register/main/reg | grep $ipsaya | awk '{print $3}')
